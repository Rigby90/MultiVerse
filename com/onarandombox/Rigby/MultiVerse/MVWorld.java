...
public class MVWorld {
	...
	private void setupSpawn() {
		String[] spawn = (this.configuration.getString("worlds." + name
				+ ".spawn", "")).split(":");
		if (spawn.length != 5) {
			this.spawn = world.getSpawnLocation();
		} else {
			this.spawn = createLocationFromString(world, spawn[0], spawn[1],
					spawn[2], spawn[3], spawn[4]);
		}
		saveAll();
	}

	private Location createLocationFromString(World world, String xStr,
			String yStr, String zStr, String yawStr, String pitchStr) {
