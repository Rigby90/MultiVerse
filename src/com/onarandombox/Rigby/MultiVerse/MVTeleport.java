...
public class MVTeleport {
	...
	public Boolean canEnterWorld(Player p, World w) {
		...
		for (int i = 0; i < whiteList.size(); i++)
			if (whiteList.get(i).contains("g:")	&& MultiVerse.Permissions.inGroup(p.getName(), whiteList.get(i).split(":")[1])) {
				returnValue = true;
				break;
			}

		for (int i = 0; i < blackList.size(); i++)
			if (blackList.get(i).contains("g:") && MultiVerse.Permissions.inGroup(p.getName(), blackList.get(i).split(":")[1])) {
				returnValue = false;
				break;
			}
